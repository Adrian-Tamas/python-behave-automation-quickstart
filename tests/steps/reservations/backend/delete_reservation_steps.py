...
# WHENs
@when('I do a delete request for reservations using book id')
...
@when('I do a delete request for reservations using user id')
...
@when('I try to delete reservations using wrong book id')
...
@when('I try to delete reservations using wrong user id')
...
# THENs
@then('I successfully deleted the reservations')
def then_i_deleted_successfully_the_reservation(context):
    assert_that(context.response.status_code).is_equal_to(200)
    assert_that(context.response.json()).is_equal_to(f'Deleted reservations for {context.entity} {context.id}')


@then('I successfully deleted reservations for user')
...
@then("I get an error that the reservations wasn't found")
